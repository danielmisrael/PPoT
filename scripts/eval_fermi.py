...
def errors(x: list, y: list) -> torch.FloatTensor:
    if not torch.is_tensor(x): x = torch.tensor(x)
    if not torch.is_tensor(y): y = torch.tensor(y)
    return torch.concatenate((torch.abs(x-y).reshape(-1, 1), torch.sqrt(x*x-y*y).reshape(-1, 1)), dim=-1)
...
def compute_scores(R_exp: list, R_llm: list, R_gt: torch.FloatTensor, stdout: bool = False,
    ...
    R_gt = R_gt[:len(R_exp)]
    R_exp, R_llm = torch.tensor(R_exp), torch.tensor(R_llm)
    ...
    M = {"last score(E_p[X])": S_exp[-1].item(), "last score(LLM)": S_llm[-1].item(),
         ...
         "last abs_error(E_p[X])": E_exp[-1,0].item(), "last abs_error(score(LLM))": E_llm[-1,0].item(),
         "avg abs_error(E_p[X])": E_avg_exp[0].item(), "avg abs_error(score(LLM))": E_avg_llm[0].item(),
