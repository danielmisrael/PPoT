...
class Program:
    ...
    def __init__(self, C: str, X: list, P: list):
        ...
        elif all(x.shape == P[0].shape for x in P): self.homogenous, self.P_tensor = torch.vstack(P)
        else: self.homogenous, self.P_tensor = False, None

    def sample(self) -> str:
        "Returns a deterministic program sampled from this probabilistic program."
        # Sample values.
        V = [x.item() for x in torch.argmax(self.P_tensor + self.G.sample(self.P_tensor.shape), dim=-1)] \
            if self.homogenous else [torch.argmax(p + self.G.sample(p.shape)).item() for p in self.mapping.values()]
        # Output code.
